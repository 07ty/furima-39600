...
describe 'ユーザー新規登録' do
  ...
  context '異常形' do
    ...
    it 'パスワードが6文字未満では登録できない' do
      @user.password = '1234'
      @user.valid?
      expect(@user.errors.full_messages).to include("Password id short (minimum is 6 characters)")
    end
    ...
    it '数字のみのパスワードでは登録できない' do
      ...
      @user.valid?
      expect(@user.errors.full_messages).to include("Password id invalid. Include both letters and numbers")
    end
    it '全角文字を含むパスワードでは登録できない' do
      @user.password = 'ａｂｃｄ123'
      @user.valid?
      expect(@user.errors.full_messages).to include("Password is invalid. Include both letters and numbers")
    end
